...
- 2026-08-04（**串到商品上架流程系統**：編輯彈窗商品卡「🚀 送到上架流程」＋列表明細列「🚀 上架」按鈕，把該商品整包資料**＋該廠商的產品報價單 Storage 路徑**（`billing.quotationPath`／`quotationName`；存摺照片不帶，屬財務資料）base64url 塞進 `?import=` 開 https://derlife-launch-flow.web.app ，那邊自動帶進「新增商品」並在建立後存進專案 `source` 欄位供後續接力的人檢視；編輯彈窗按鈕讀的是「當下輸入框」的值，改完不用先存檔也帶得走；網址超過 7500 字元會擋下並提示精簡成分／文案）
